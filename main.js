...
const members = [
    ...
    { id: 'sub-writer', name: 'Writer', x: 5.5, y: 6.5, color: '#4d94ff', role: '文案代理', status: 'idle' },
    { id: 'sub-n8n', name: 'N8N小幫手', x: 1, y: 7, color: '#4dff88', role: '自動化代理', status: 'offline' },
    { id: 'sub-alex', name: 'Alex', x: 7.5, y: 2, color: '#f0ff4d', role: '系統開發', status: 'idle' }
];
...
function drawMember(member) {
    ...
    ctx.fillStyle = member.isBoss ? 'rgba(255, 215, 0, 0.9)' : (member.id === 'looploom' ? 'rgba(138, 43, 226, 0.8)' : (member.id === 'signalscout' ? 'rgba(34, 139, 34, 0.8)' : 'rgba(255, 77, 77, 0.8)'));
...
setInterval(() => {
    ...
    const actions = member.isBoss ? bossActions : (member.id === 'looploom' ? spiderActions : (member.id === 'signalscout' ? chameleonActions : ['正在巡視龍蝦牆', '正在沙發區休息', '正在檢查自動化腳本', '正在測試新功能']));
